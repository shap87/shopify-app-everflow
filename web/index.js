...
export async function createServer(
  ...
  //   storage: multerS3({
  //     s3: s3,
  //     bucket: 'dermeleve-video-testimonial',
  ...
  app.post("/api/proxy/uploadFile", multer().single('video'), async (req, res) => {
    let status = 200;
    let error = null;
    let data = null;
    let message = null;
    ...
    try {
      ...
      if(req.file){
        ...
        const params = {
          Bucket: 'dermeleve-video-testimonial',
          Key: fileName, // req.file.key, // Use the key generated by Multer-S3
          Body: fileData,
          // ACL: 'public-read',
        };
        ...
        res.status(500).send({ success: status === 200, error: true, data, message: 'No file provided' });
      }
    } catch (error) {
      console.log('/api/proxy/uploadFile ', error)
      res.status(500).send({ success: status === 200, error, data, message });
    }
  });
